<template>
  <div class="repairs-page">
    ...
    <!-- Desktop User Icon -->
    <div class="desktop-user-icon" @click="toggleSidebar" v-if="currentAdmin">
...
onMounted(async () => {
  ...
  const repairmen = JSON.parse(localStorage.getItem('admin') || '[]');
  const foundAdmin = repairmen.find(r => r.id == currentAdminId);
  if (foundAdmin) {
    currentAdmin.value = {
      id: foundAdmin.id,
      fullName: foundAdmin.fullName || foundAdmin.name,
      phone: foundAdmin.phone,
      specialty: foundAdmin.specialty || 'general',
...
const handleLogout = async () => {
  ...
  if (confirm('آیا از خروج از حساب کاربری خود اطمینان دارید؟')) {
    ...
    if (adminId) {
      localStorage.removeItem(`repair_parts_${adminId}`)
    }

    // Clear the current Admin data from memory
